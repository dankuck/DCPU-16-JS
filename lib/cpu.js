...
CPU.prototype = {
    ...
    reset: function() {
        ...
        for(i = 0; i < root.REGISTER_NAMES.length; ++i) {
            this.set(root.REGISTER_NAMES[i], 0);
        }
        for(i = 0; i < this.ramSize; ++i) {
            this.set(i, 0);
        }
        for(i = 0; i < this._devices.length; i++) {
            if(typeof this._devices[i].reset === 'function') this._devices[i].reset();
        }
    ...
    removeDevice: function(device){
    	if (typeof device.onDisconnect === 'function')
    		device.onDisconnect(this);
    	for (var i = this._devices.length - 1; i >= 0; i--)
    		if (this._devices[i] === device)
    			this._devices.splice(i, 1);
    },
    removeDevices: function(){
    	for (var i = this._devices.length - 1; i >= 0; i--)
    		this.removeDevice(this._devices[i]);
    },
